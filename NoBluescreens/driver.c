...
VOID KeHookedBugCheckEx(ULONG BugCheckCode, ULONG_PTR Code1, ULONG_PTR Code2,
	...
	DbgPrint("KeBugCheckEx called by process %d, thread %d\n", PsGetCurrentProcessId(), PsGetCurrentThreadId());
...
VOID DriverUnload(PDRIVER_OBJECT DriverObject) {
	...
	if (Status != STATUS_SUCCESS)
		...
		DbgPrint("Restored the orignal KeBugCheckEx function\n");

	DbgPrint("Unloaded NoBluescreens\n");
}
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject,
	...
	DbgPrint("NoBluescreens entry\n");
	DbgPrint("KeBugCheckEx address: 0x%llx\n", KeBugCheckExAddress);
	DbgPrint("KeHookedBugCheckEx address: 0x%llx\n", KeHookedBugCheckEx);
